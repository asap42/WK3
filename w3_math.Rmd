...
Math Homework Week 3: Chapter 7 
Excercises 1 through 10

1. Normal distribution m = 35, sd = 10
a) greater than 34?
```{r}
...
2. The mean and the standard deviation of a normal distribution are: mean = 0, sd = 1
b)
```{r}
m <- mean(0,8)
m <- m + 75
...
3. The normal distribution is defined by the mean and the standard deviation
4.
5. a) What score would be needed to be in the 85th percentile?
```{r}
...
```
6. 
7. Normal distribution, mean = 20, sd = 4. Find the Z
...
print(d)
```
8. Normal distribution, mean = 71, sd = 8
...
print(c)
```
9. Normal distribution, mean = 120, sd = 5. What is the probability it is above 127?
```{r}
x <- pnorm(127, 120, 5)
print(x)
```
...
6. correlation = 0.60, n = 28, Find Probability if sample correlation > 0.65?
```{r}
val1 <- .5 * log((1+.6)/(1-.6))
print(val1)
...
print(p)
```
7. What would be the shape of the frequency distribution?
The shape would be a bell-shaped curve. 

8. Normal distribution, mean = 20, sd = 10. What is the probability that the difference score will be greater than 5?
```{r}
x <- (10^2)/2
print(x)
e <- sqrt(x)
...
print(p)
```
9. What is the shape of the sampling distribution of r? 
...
10. Normal distribution, mean = 0, sd = 1, what is the probability it will be 0.5?
```{r}
p <- dnorm(.5, mean = 0, sd = 1)
print(p)
```

11. Normal distribution, mean = 120, sd = 5. Four scores are randomly sampled. What is the probability that the mean of the four scores is above 127?
```{r}
rsamples <- (5^2)/4
print(rsamples)
s <- sqrt(rsamples)
...
12. Coin Flip. n = 25, mean = 0.64 Compute the 95% confidence interval on the proportion of times this person can predict coin flips correctly.
```{r}
